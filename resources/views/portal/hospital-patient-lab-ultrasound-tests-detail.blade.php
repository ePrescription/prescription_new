...
<style>
    ...
    <form action="#" role="form" method="POST" class="display form-horizontal ">
        ...
        @if(count($ultraSound)>0)
            <h4 class="m-t-0 m-b-30">Ultra Sound Test Details</h4>
            @foreach($ultraSound as $recentTest)
                <?php $displaySet=0; ?>
    ...
    <?php /* ?>
        ...
        @foreach($ultraSound as $ultraSoundValue)
            @if($ultraSoundValue->isValueSet==1)
            <div class="form-group col-sm-6">
                <label class="col-sm-12 control-label">{{$ultraSoundValue->examinationName}}</label>
            </div>
            <?php $i++; ?>
            @endif
        @endforeach
